...
set T := 1..18;           # Teams
param pref{T};          # Personal team preference
...
var sl_G{T} binary;                 # slack variable for active teams
...
# all goalies from same team
# active teams
s.t. goalieTeams{t in T}: sum{k in name_G: t_G[k]=t} 0.3*x_G[k] <= sl_G[t];
s.t. allGoalesfromOneTeam: sum{t in T} sl_G[t] <= 1;
# take top goalies
s.t. topGoalies{k1 in name_G, k2 in name_G: t_G[k1]=t_G[k2]}: (x_G[k2]-x_G[k1])*p_G[k1] <= x_G[k2]*p_G[k2];

# number of golies in starting squad
s.t. startingGoaliesUp: sum{k in name_G} s_G[k] <= 1;
s.t. startingGoaliesLo: sum{k in name_G} s_G[k] >= 1;

# number of defenders in starting squad
s.t. startingDefendersUp: sum{k in name_D} s_D[k] <= 4;
s.t. startingDefendersLo: sum{k in name_D} s_D[k] >= 4;

# number of midfielders in starting squad
s.t. startingMidfieldersUp: sum{k in name_M} s_M[k] <= 5;
s.t. startingMidfieldersLo: sum{k in name_M} s_M[k] >= 5;

# number of strikers in starting squad
s.t. startingStrikersUp: sum{k in name_S} s_S[k] <= 3;
s.t. startingStrikersLo: sum{k in name_S} s_S[k] >= 3;

# 13 Freunde muesst ihr sein!
s.t. elfFreunde: sum{k in name_G} s_G[k] 
                + sum{k in name_D} s_D[k] 
                + sum{k in name_M} s_M[k] 
                + sum{k in name_S} s_S[k] = 13;

solve;

printf "==========================================================\n";
printf "|                                                        |\n";
printf "|  Ergebnis der Optimierung fuer die 13 besten Spieler:  |\n";
printf "|                                                        |\n";
printf "==========================================================\n";
