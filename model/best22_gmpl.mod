...
maximize lastSeasonsPoints: sum{k in name_G} g_G[k] * x_G[k] * pref[t_G[k]] * w_G[k] +
                            sum{k in name_D} g_D[k] * x_D[k] * pref[t_D[k]] * w_D[k] +
                            sum{k in name_M} g_M[k] * x_M[k] * pref[t_M[k]] * w_M[k] +
                            sum{k in name_S} g_S[k] * x_S[k] * pref[t_S[k]] * w_S[k];
...
printf "==========================================================\n";
printf "|                                                        |\n";
printf "|  Ergebnis der Optimierung fuer die 22 besten Spieler:  |\n";
printf "|                                                        |\n";
printf "==========================================================\n";
printf "\nPunkte in der Vorsaison: %i\n", sum{k in name_G} g_G[k] * x_G[k]
                                         +sum{k in name_D} g_D[k] * x_D[k]
                                         +sum{k in name_M} g_M[k] * x_M[k]
                                         +sum{k in name_S} g_S[k] * x_S[k];

#Writing this gives an error: invalid reference to objective lastSeasonsPoints, wtf?!										 
#printf "\nGewichtete Punkte in der Vorsaison: %i\n", lastSeasonsPoints;									 
printf "\nGewichtete Punkte in der Vorsaison: %i\n", sum{k in name_G} g_G[k] * x_G[k] * pref[t_G[k]] * w_G[k]
													+sum{k in name_D} g_D[k] * x_D[k] * pref[t_D[k]] * w_D[k]
													+sum{k in name_M} g_M[k] * x_M[k] * pref[t_M[k]] * w_M[k]
													+sum{k in name_S} g_S[k] * x_S[k] * pref[t_S[k]] * w_S[k];
